...
.PARAMETER V3API
    ...
    Needed if using the V3 API for Update-IdentityNowSource due to different object models between API versions.
        ...
        try {
            if ($sourceID) {
                if ($accountProfiles) {
                    ...
                    if ($V3API) {
                        $IDNSources = Invoke-RestMethod -Method Get -Uri "https://$($IdentityNowConfiguration.orgName).api.identitynow.com/v3/sources/$($sourceID)" -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)" }
                    }
                    else {
                ...
                if ($V3API) {
                    $IDNSources = Invoke-RestMethod -Method Get -Uri "https://$($IdentityNowConfiguration.orgName).api.identitynow.com/v3/sources" -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)" }
                }
                else {
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUNCu2c3XeRXau7khRLIZ47Tvz
# dJ2gggqNMIIFMDCCBBigAwIBAgIQBAkYG1/Vu2Z1U0O1b5VQCDANBgkqhkiG9w0B
...
# BAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFLKyfFSTgsLq
# Yj7LDEeQBLqv31H4MA0GCSqGSIb3DQEBAQUABIIBAEebNvEWeKJA0SIjTAbgv14h
# qnscMEBaEUZMEhtFpjvnOsJWDBrKUmasrKthDDo1t++Y4Nr3SxgSk0EcROD7dy+U
# vskfAJhuaSAApPMMSM9QvyDSJBlczdLJP+WTa8DGqju+uy2CW//GFf5u3pq1Mwwc
# 5+IGXUVn3G1nB5g4lIGR7adeVEGEtAKCnaKzr9TYbx/qD/skRfu2BZT3Y+W0+1AK
# 79bA2RxtUO9SQv88oVOxH6wkBCUltPqoAll+MpQ1mGegRh+uik+iRnkyiapoCAzZ
# UVZrVcL1nY9xeIhD5XfJsZ5EDV7hiCwR9PoyU8H7hQoRUtqV9uaYjuPSm3QAJhc=
